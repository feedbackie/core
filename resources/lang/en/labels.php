<?php

return [
    'general' => [
        'yes' => 'Yes',
        'no' => 'No',
        'created_at' => 'Created at',
    ],

    'filters' => [
        'answer' => 'Answer',
        'created_from' => 'Created From',
        'created_until' => 'Created Until',
        'has_comment' => 'Has Comment',
        'has_email' => 'Has Email',
        'has_options' => 'Has Options',
        'options' => 'Options',
        'option' => 'Option',
        'url' => 'URL',
    ],

    'periods' => [
        'today' => 'Today',
        'yesterday' => 'Yesterday',
        'current_week' => 'Last Week',
        'previous_week' => 'Previous Week',
        'current_month' => 'Current Month',
        'previous_month' => 'Previous Month',
        'current_year' => 'Current Year',
        'previous_year' => 'Previous Year',
    ],

    'options' => [
        'resolved_my_issue' => 'Resolved my issue',
        'clear_instructions' => 'Clear instructions',
        'easy_to_follow' => 'Easy to follow',
        'no_jargon' => 'No jargon',
        'no_mistakes' => 'No mistakes',
        'pictures_helped' => 'Pictures helped',
        'other' => 'Other',
        'article_is_outdated' => 'Article is outdated',
        'incorrect_instructions' => 'Incorrect instructions',
        'too_technical' => 'Too technical',
        'not_enough_information' => 'Not enough information',
        'not_enough_pictures' => 'Not enough pictures',
        'too_many_grammar_mistakes' => 'Too many grammar mistakes',
        'bad_color_scheme' => 'Bad color scheme',
    ],

    'language_scores' => [
        'very_unsatisfied' => 'Very unsatisfied',
        'unsatisfied' => 'Unsatisfied',
        'neutral' => 'Neutral',
        'satisfied' => 'Satisfied',
        'very_satisfied' => 'Very satisfied',
    ],

    'pages' => [
        'overview' => [
            'title' => 'Overview',
        ],
    ],

    'metadata' => [
        'metadata_fieldset' => 'Metadata',
        'ip' => 'IP Address',
        'os' => 'Operating System',
        'country' => 'Country',
        'device' => 'Device',
        'browser' => 'Browser',
        'language' => 'Language',
        'user_agent' => 'User Agent',
        'duration' => 'Duration',
    ],

    'actions' => [
        'remove' => 'Remove',
        'overview' => 'Overview',
        'code' => 'Code'
    ],
    'resources' => [
        'feedback' => [
            'navigation_group' => 'Feedback',
            'navigation_label' => 'All Answers',
            'record_label' => 'Feedback',
            'record_plural_label' => 'Feedback',
            'main_fieldset' => 'Feedback',
            'url' => 'Url',
            'answer' => 'Answer',
            'options' => 'Options',
            'language_score' => 'Language Quality Score',
            'comment' => 'Comment',
            'email' => 'Email',
        ],

        'feedback_stats' => [
            'navigation_group' => 'Feedback',
            'navigation_label' => 'Feedback Stats',
            'record_label' => 'Feedback',
            'record_plural_label' => 'Feedback',
            'url' => 'Url',
            'total' => 'Total',
            'yes_count' => 'Answered "Yes"',
            'no_count' => 'Answered "No"',
            'options' => 'Options',
            'avg_score' => 'Avg Language Score',
            'language_scores' => 'Language Scores',
            'comments' => 'Comments',
            'total_feedback_count' => 'Total Answers',
            'selected_options_count' => 'Has Selected Options',
            'feedback_answers' => 'Feedback Answers',
        ],

        'report' => [
            'navigation_group' => 'Reports',
            'navigation_label' => 'All Reports',
            'record_label' => 'Report',
            'record_plural_label' => 'Reports',
            'main_fieldset' => 'Report',
            'url' => 'Url',
            'full_text' => 'Full Text',
            'diff_text' => 'Fixed Text',
            'comment' => 'Comment',
            'email' => 'Email',
        ],

        'report_stats' => [
            'navigation_group' => 'Reports',
            'navigation_label' => 'Reports Stats',
            'record_label' => 'Report',
            'record_plural_label' => 'Reports',
            'url' => 'Url',
            'total' => 'Total',
            'has_comment_count' => 'Have Comment',
            'total_reports_count' => 'Total Reports',
            'archived_reports_count' => 'Archived Reports',
        ],

        'sites' => [
            'navigation_label' => 'Sites',
            'record_label' => 'Site',
            'record_plural_label' => 'Sites',
            'code' => 'Code',
            'name' => 'Name',
            'domain' => 'Domain',
            'settings' => 'Settings',
            'feedback_enabled' => 'Feedback Enabled',
            'reports_enabled' => 'Reports Enabled',
            'id' => 'ID',
            'last_event' => 'Last Event',
            'reports_count' => 'Report Count',
            'feedback_count' => 'Feedback Count',
        ]
    ]
];
